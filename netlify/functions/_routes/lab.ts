...
// Tudo que a squad criou (qualquer status) + os aprovados que ela pode usar.
app.get("/", async (c) => {
  const me = c.get("me");
  const db = await getDb();
  const tools = (await db.select().from(s.tool)).filter((t: any) => t.squadId === me.squadId || t.criadoPor === me.id);
  const mcps = (await db.select().from(s.conexaoMcp)).filter((m: any) => (m.squadId === me.squadId || m.criadoPor === me.id) && m.escopo !== "global");
